...
async function main() {
  ...
  registrySymbol = "TR";
  votesName = "Test Votes";
  votesSymbol = "TV";
  votesVersion = "v0.1.0";

  amountOfEtherPerPerson = 200000;
  numPeople = 21;
  const sendValue = ethers.utils
    .parseEther(amountOfEtherPerPerson.toString())
    .mul(21);
  ...
  CommunityRegistryFactory = await ethers.getContractFactory(
    "CommunityRegistry"
  );
  communityRegistry = await CommunityRegistryFactory.deploy(
    registryName,
    registrySymbol
  );
  ...
  communityVotes = await CommunityVotesFactory.deploy(
    votesName,
    votesSymbol,
    votesVersion
  );
  ...
  CommunityOnboardingFactory = await ethers.getContractFactory(
    "CommunityOnboarding"
  );
  communityOnboarding = await CommunityOnboardingFactory.deploy(
    communityName,
    communityCoin.address,
    communityRegistry.address,
    communityVotes.address,
    { value: sendValue }
  );
  ...
  await communityCoin.grantRole(
    communityCoin.callStatic.CONVERTER_ROLE(),
    communityOnboarding.address
  );
  await communityRegistry.grantRole(
    communityRegistry.callStatic.REGISTERER_ROLE(),
    communityOnboarding.address
  );
  await communityVotes.grantRole(
    communityVotes.callStatic.GRANTER_ROLE(),
    communityOnboarding.address
  );
  ...
  communityOnboarding.grantRole(
    communityOnboarding.callStatic.ONBOARDER_ROLE(),
    process.env.ONBOARDER
  );
